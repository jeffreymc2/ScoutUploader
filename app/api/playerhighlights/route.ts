...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    let nextUrl = `${DRUND_API_PATH}/~/highlights/${encodeURIComponent(playerID)}/?page=${page}&limit=${limit}&type=${type}&version=v2`;
    ...
    while (nextUrl) {
      ...
      if (highlightsResponse.ok) {
        ...
        return NextResponse.json(
          { message: 'Failed to fetch highlights from Drund API', error: errorData },
    ...
    if (type === 'h') {
      ...
      combinedHighlights = [...supabaseVideos, 
