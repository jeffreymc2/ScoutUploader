// This component is used to build a playlist of highlight videos. It allows the user to drag and drop videos between two lists, one for videos that have not been added to the playlist and one for videos that have been added to the playlist. The user can save the playlist to the database if they are logged in.
// The component uses the DndContext component from the @dnd-kit/core library to handle drag and drop functionality. It also uses the SortableContext and SortableHighlightVideoItem components from the @dnd-kit/sortable library to create the sortable lists of videos.
// The component fetches the saved playlist from the database when it mounts and updates the playlist state accordingly. It also updates the saved playlist state when the user adds or removes videos from the playlist.
// The component uses the useUser hook to get the current user from the auth context. It also uses the useRouter hook from the next/navigation library to refresh the page after saving the playlist.
