...
const MediaRenderer: React.FC<MediaRendererProps> = ({ file }) => {
  ...
  useEffect(() => {
    if (file.isVideo) {
      const compressedVideoPath = file.image.replace(".mp4", "_compressed.mp4");
      const thumbnailPath = file.image.replace(".mp4", "_thumbnail.jpg");
  
      // Check if the compressed video exists
      const { data: compressedVideoData } = supabase.storage
        .from("media")
        .getPublicUrl(compressedVideoPath);
  
      if (compressedVideoData.publicUrl) {
        // Use the compressed video URL
        setThumbnailUrl(compressedVideoData.publicUrl);
      } else {
      ...
      if (thumbnailData.publicUrl) {
        ...
        setThumbnailUrl(thumbnailData.publicUrl);
      }
    } else {
...
//         .from('media')
//         .getPublicUrl(compressedVideoPath)
//         .then(({ data: compressedVideoUrl, error: compressedVideoError }) => {
//           if (!compressedVideoError && compressedVideoUrl) {
//             // Use the compressed video URL
//             setThumbnailUrl(compressedVideoUrl.publicUrl);
//           } else {
//             // Fallback to the original video URL
//             setThumbnailUrl(file.image);
//           }
...
//         .from('media')
//         .getPublicUrl(thumbnailPath)
//         .then(({ data: thumbnailUrl, error: thumbnailError }) => {
//           if (!thumbnailError && thumbnailUrl) {
//             // Use the thumbnail URL
//             setThumbnailUrl(thumbnailUrl.publicUrl);
//           }
//         });
//     } else {
//       // Directly set the thumbnailUrl for non-video files
